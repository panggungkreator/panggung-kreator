...
export default function CheckoutClient({ selectedPackage }: { selectedPackage: any }) {
  ...
  if (loadingSession) {
    return (
      <div className="min-h-screen bg-white text-[#2c2c2c] dark:bg-[#2c2c2c] dark:text-white flex items-center justify-center font-sans">
        <div className="text-center">
          <svg className="animate-spin h-8 w-8 text-black dark:text-white mx-auto mb-4" fill="none" viewBox="0 0 24 24">
            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
          </svg>
          <p className="text-zinc-500 dark:text-zinc-400 text-xs font-mono uppercase tracking-widest animate-pulse">Memuat sesi pendaftaran...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white text-[#2c2c2c] dark:bg-[#2c2c2c] dark:text-white font-sans transition-colors duration-300 selection:bg-[#2c2c2c] selection:text-white dark:selection:bg-white dark:selection:text-[#2c2c2c]">

      <main className="max-w-5xl mx-auto px-4 py-6 md:py-8">
        <div className="flex items-center gap-3.5 mb-6 md:mb-8 pb-4 border-b border-[#2c2c2c]/10 dark:border-white/10">
          <button
            type="button"
            onClick={() => router.back()}
            className="w-9 h-9 rounded-full border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-800/80 flex items-center justify-center hover:bg-zinc-100 dark:hover:bg-zinc-700 text-zinc-600 dark:text-zinc-300 hover:text-zinc-900 dark:hover:text-white transition-colors cursor-pointer shrink-0"
            title="Kembali"
          >
            <ArrowLeft size={16} />
          </button>
          <div>
            <h1 className="font-sans text-2xl md:text-3xl font-black uppercase tracking-tighter text-[#2c2c2c] dark:text-white leading-[0.95]">
              PENDAFTARAN PANGGUNG KREATOR AKADEMI
            </h1>
            <p className="font-sans text-[11px] md:text-xs uppercase tracking-wider text-[#2c2c2c]/60 dark:text-white/60 mt-1.5 leading-relaxed font-medium">
              {qrisGenerated
          ...
          qrisGenerated ? (
              ...
              <div className="w-full lg:w-7/12 flex flex-col justify-between space-y-4 animate-fade-in transition-colors duration-300 lg:border-r lg:border-zinc-200 dark:lg:border-zinc-800 lg:pr-8">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-3 border-b border-zinc-200 dark:border-white/10 pb-3">
                    <div className="w-5 h-5 rounded-none bg-black dark:bg-white text-white dark:text-black flex items-center justify-center text-[10px] font-bold">
                      ✓
                    </div>
                    <div>
                      <h2 className="font-title font-bold text-xs uppercase tracking-[0.2em] text-zinc-900 dark:text-white">
                        Pendaftaran Berhasil Diproses
                      </h2>
                      <p className="text-[10px] text-zinc-400 uppercase tracking-wider font-mono">Akun Anda sedang disiapkan di sistem</p>
                    </div>
                  </div>

                  {/* Rincian Tagihan & Nominal */}
                  <div className="bg-zinc-50 dark:bg-zinc-900/60 rounded-none p-4 border border-zinc-300 dark:border-zinc-700 text-xs">
                    <div className="flex justify-between items-center mb-2 font-bold text-zinc-900 dark:text-white uppercase tracking-wider">
                      <span>Panggung Kreator Akademi</span>
                      <span className="font-mono text-[11px]">({selectedPackage?.name || "Paket Advanced"})</span>
                    </div>
                    <div className="space-y-1.5 text-zinc-600 dark:text-zinc-400 text-xs font-mono">
                    ...
                    <div className="border-t border-zinc-200 dark:border-zinc-800 my-2.5 pt-2.5 flex justify-between items-center font-bold text-zinc-900 dark:text-white text-sm">
                      <span className="uppercase tracking-wider text-xs">Total Transfer</span>
                      <span className="text-[#bc151b] text-base font-mono">Rp {finalPrice.toLocaleString('id-ID')}</span>
                    </div>
                  </div>

                  {/* QRIS Code (Mobile Only - Tampil di bawah Total Transfer) */}
                  <div className="lg:hidden mt-2 border border-zinc-200 dark:border-zinc-800 p-2">
                    {renderQrisBlock(true)}
                  </div>

                  {/* Langkah Pembayaran */}
                  <div className="text-xs text-zinc-600 dark:text-zinc-400 space-y-2 border-t border-zinc-200 dark:border-zinc-800 pt-3">
                    <div className="flex gap-2">
                      <span className="font-bold text-zinc-900 dark:text-white font-mono">01.</span>
                      <span>Transfer tepat <strong className="text-red-600 font-mono font-bold">Rp {finalPrice.toLocaleString('id-ID')}</strong> (termasuk kode unik).</span>
                    </div>
                    <div className="flex gap-2">
                      <span className="font-bold text-zinc-900 dark:text-white font-mono">02.</span>
                      <span>Ambil screenshot bukti transfer sukses Anda.</span>
                    </div>
                    <div className="flex gap-2">
                      <span className="font-bold text-zinc-900 dark:text-white font-mono">03.</span>
            ...
            <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
              ...
              <div className="w-full lg:w-3/5">
                <div className="bg-white dark:bg-[#1a1a1a] border border-zinc-300 dark:border-zinc-700 rounded-none p-6 md:p-8 transition-colors duration-300">
                  <h2 className="font-title font-bold text-xs uppercase tracking-[0.2em] text-zinc-900 dark:text-white mb-6 flex items-center gap-3 border-b border-zinc-300 dark:border-zinc-700 pb-4">
                    <span className="w-5 h-5 rounded-none bg-black dark:bg-white text-white dark:text-black flex items-center justify-center text-[10px] font-bold font-mono">1</span>
                    Informasi Personal
                  </h2>

                  {error && (
                    <div className="mb-6 p-4 bg-red-500/10 border border-red-500/30 rounded-none text-xs text-[#bc151b] flex items-center gap-2 font-medium animate-fade-in">
              ...
              <div className="w-full lg:w-2/5">
                <div className="bg-white dark:bg-[#1a1a1a] border border-zinc-300 dark:border-zinc-700 rounded-none p-5 md:p-6 sticky top-28 transition-colors duration-300">
                  <h2 className="font-title font-bold text-xs uppercase tracking-[0.2em] text-zinc-900 dark:text-white mb-6 flex items-center gap-3 border-b border-zinc-300 dark:border-zinc-700 pb-4">
                    <span className="w-5 h-5 rounded-none bg-black dark:bg-white text-white dark:text-black flex items-center justify-center text-[10px] font-bold font-mono">2</span>
                    Pembayaran
                  </h2>
                  <div className="mb-4">
                  ...
                  <div className="border-t border-b border-zinc-300 dark:border-zinc-700 py-3.5 mb-4 space-y-3">
                    {/* Subtotal, Biaya Layanan & Total Pembayaran (Moved to top) */}
                    <div className="space-y-1.5 font-mono text-xs">
                      <div className="flex justify-between items-center text-zinc-600 dark:text-zinc-400">
                        <span className="font-sans uppercase tracking-wider text-[11px]">Subtotal</span>
                        <span>Rp {basePrice.toLocaleString('id-ID')}</span>
                      </div>
                      {appliedVoucher && (
                        <div className="flex justify-between items-center text-emerald-600 dark:text-emerald-400 font-semibold">
                          <span className="font-sans uppercase tracking-wider text-[11px]">Diskon ({appliedVoucher.code})</span>
                          <span>- Rp {appliedVoucher.discountNominal.toLocaleString('id-ID')}</span>
                        </div>
                      )}
                      {uniqueCode > 0 && (
                        <div className="flex justify-between items-center text-zinc-600 dark:text-zinc-400">
                          <span className="font-sans uppercase tracking-wider text-[11px]">Kode Unik</span>
                          <span className="text-emerald-600 dark:text-emerald-400 font-bold">+ Rp {uniqueCode.toLocaleString('id-ID')}</span>
                        </div>
                      )}
                      <div className="flex justify-between items-center text-zinc-400 dark:text-zinc-500 pb-1">
                        <span className="font-sans uppercase tracking-wider text-[11px]">Biaya Layanan</span>
                        <span>Rp 0</span>
                      </div>
                      <div className="flex justify-between items-center font-bold text-sm mt-2 text-zinc-900 dark:text-white border-t border-zinc-200 dark:border-zinc-800 pt-2.5">
                    ...
                    <div className="pb-3 border-b border-zinc-200 dark:border-zinc-800">
                      {appliedVoucher ? (
                        <div className="flex items-center justify-between bg-zinc-50 dark:bg-zinc-800/60 border border-zinc-300 dark:border-zinc-700 rounded-none px-3 py-2">
                          <span className="text-zinc-900 dark:text-white text-xs font-bold font-mono uppercase tracking-wider flex items-center gap-1.5">
                          ...
                          <button onClick={handleRemoveVoucher} className="text-zinc-500 hover:text-[#bc151b] text-[11px] font-bold uppercase tracking-wider transition-colors cursor-pointer">
                            Hapus
                          </button>
                        </div>
                      ) : !showVoucherInput ? (
                        <button
                          ...
                          className="text-[11px] font-bold uppercase tracking-wider text-zinc-600 dark:text-zinc-400 hover:text-black dark:hover:text-white transition-colors cursor-pointer flex items-center gap-1"
                        >
                          + Punya Kode Voucher?
                        </button>
                      ) : (
                        <div className="flex items-end gap-2 w-full animate-fade-in">
                          ...
                          <button
                            ...
                            className="bg-black dark:bg-white text-white dark:text-black hover:bg-[#bc151b] dark:hover:bg-[#bc151b] hover:text-white dark:hover:text-white px-4 py-2 rounded-none text-[10px] uppercase tracking-widest font-bold transition-all disabled:opacity-50 cursor-pointer h-[34px] flex items-center justify-center shrink-0 mb-0.5"
                          >
                      ...
                      {voucherMessage && (
                        <p className={`mt-1.5 text-[10px] font-mono ${voucherMessage.type === 'error' ? 'text-red-500' : 'text-emerald-500'}`}>
                          {voucherMessage.text}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="pb-1">
                      {appliedReferral && !isReferralFromUrl ? (
                        <div className="flex items-center justify-between bg-zinc-50 dark:bg-zinc-800/60 border border-zinc-300 dark:border-zinc-700 rounded-none px-3 py-2">
                          ...
                          <button onClick={handleRemoveReferral} className="text-zinc-500 hover:text-[#bc151b] text-[11px] font-bold uppercase tracking-wider transition-colors cursor-pointer">
                            Hapus
                          </button>
                        </div>
                      ) : !showReferralInput ? (
                        ...
                        <div className="flex items-end gap-2 w-full animate-fade-in">
                          <div className="flex-1">
                            <InputLine
                              label={isReferralFromUrl ? "KODE REFERRAL (DARI LINK)" : "KODE REFERRAL (OPSIONAL)"}
                              type="text"
                          ...
                          {isReferralFromUrl ? (
                            <span className="text-[10px] font-bold uppercase tracking-wider text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-800/50 rounded-none px-3 py-1.5 flex items-center shrink-0 mb-0.5 h-[34px] font-mono">
                              Terpasang
                            </span>
                          ) : (
                            <button
                      ...
                      {referralMessage && (
                        <p className={`mt-1.5 text-[10px] font-mono ${referralMessage.type === 'error' ? 'text-red-500' : 'text-blue-500'}`}>
                  ...
                  <button
                    ...
                    className="w-full py-3.5 bg-black dark:bg-white text-white dark:text-black hover:bg-[#bc151b] dark:hover:bg-[#bc151b] hover:text-white dark:hover:text-white font-bold text-[11px] uppercase tracking-widest rounded-none transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer border border-transparent"
                  >
                    {isSubmitting ? (
                  ...
                  <div className="mt-3.5 flex items-start gap-2 text-[10px] text-zinc-500 font-mono leading-normal">
